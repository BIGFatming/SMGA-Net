import torch
import cv2
from models.peppa_landmark.slim import Slim
import numpy as np
from models.peppa_landmark.tracker import Tracker
from models.peppa_landmark.headpose import get_head_pose


class Detector:
    def __init__(self, detection_size=(160, 160), device=torch.device('cuda:0')):
        self.model = Slim()
        self.device = device
        self.model.load_state_dict(torch.load('models/peppa_landmark/slim_160_latest.pth', map_location=device))
        self.model.eval()
        # self.model.cuda()
